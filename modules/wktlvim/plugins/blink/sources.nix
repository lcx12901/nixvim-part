    ...
    default.__raw = ''
      function(ctx)
        ...
        if success and node and vim.tbl_contains({ 'comment', 'line_comment', 'block_comment' }, node:type()) then
          ...
          ${lib.optionalString config.plugins.blink-cmp-dictionary.enable "table.insert(comment_sources, 'dictionary')"}
          return comment_sources
        elseif vim.bo.filetype == 'gitcommit' then
          local git_sources = { 'buffer', 'spell' }
          ${lib.optionalString config.plugins.blink-cmp-dictionary.enable "table.insert(git_sources, 'dictionary')"}
